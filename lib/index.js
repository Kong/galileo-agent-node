...
module.exports = function Agent (serviceToken, environment, options) {
  ...
  debug('[%s] plugin options: %s', chalk.yellow('agent'), JSON.stringify(self.opts, null, ' '))
  ...
  return function (req, res, next) {
    ...
    if (self.opts.logBody) {
      req.on('end', function () {
        ...
        bodies.req.size = Buffer.byteLength(body)
        bodies.req.base64 = body.toString('base64')
      })
    }
    ...
    res.end = function (data, encoding) {
      ...
      // construct body
      bodies.res.size = Buffer.byteLength(data)
      bodies.res.base64 = bodies.res.size ? data.toString('base64') : ''
      ...
      if (bodies.res.size && resHeaders.headersObj['content-encoding'] === 'gzip') {
        var inflatedBuffer = zlib.gunzipSync(data)
        bodies.res.base64 = inflatedBuffer.toString('base64')
      }
