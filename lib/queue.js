...
var Queue = module.exports = function (serviceToken, environment, options) {
  ...
  this.queue = async.queue(function (entry, done) {
    ...
    // start timer
    if (!self.timeout) {
      debug('[%s] timeout set (%d)', chalk.yellow('agent'), self.opts.queueTimeout)
      self.timeout = setTimeout(function () {
        debug('[%s] timeout triggered (%d)', chalk.yellow('agent'), self.opts.queueTimeout)
        self.flush()
      }, self.opts.queueTimeout)
    }

    // throttle
    if (self.alf.har.log.entries.length >= self.opts.queueEntries) {
      self.flush()
      return done()
    }

    debug('[%s] queued (%d/%d)', chalk.yellow('agent'), self.alf.har.log.entries.length, self.opts.queueEntries)
    done()
  })
