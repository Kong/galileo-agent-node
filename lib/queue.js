...
function writeErrorsToDisk (response, postData, opts) {
  ...
  if (pathExists && failPath.isDirectory()) {
    ...
    var time = new Date()
    var failMessage = '{\n\t"date": "'
    failMessage += time.toString()
    failMessage += '",\n\t"unix": '
    failMessage += Math.round(time.getTime() / 1000)
    failMessage += ',\n\t"collector_response": '
    failMessage += JSON.stringify(response)
    failMessage += ',\n\t"agent_options": '
    failMessage += JSON.stringify(opts)
    failMessage += ',\n\t"data_to_post": '
    failMessage += postData
    failMessage += '\n}\n'
