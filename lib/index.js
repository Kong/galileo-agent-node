...
module.exports = function Agent (serviceToken, environment, options) {
  ...
  self.opts = extend({
    ...
    limits: {
      bodySize: 1000, // bytes
      retry: 0, // RETRY_COUNT agent spec
  ...
  return function (req, res, next) {
    ...
    if (self.opts.logBody) {
      req.on('end', function () {
        ...
        bodies.req.base64 = helpers.b64.enc(bodies.req.base64)
      })
    }
    ...
    res.end = function (data, encoding) {
      ...
      bodies.res.base64 = helpers.b64.enc(bodies.res.base64)
      ...
      if (self.opts.logBody && !reqEndFired) { // only run this if req.on('end') has not fired yet
        ...
        req.on('end', function () {
          entry.request.postData.text = self.opts.logBody ? bodies.req.base64 : ''
