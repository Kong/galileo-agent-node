...
module.exports = function Agent (serviceToken, options) {
  ...
  this.queue = async.queue(function (entry, done) {
    ...
    // throttle
    if (self.message.har.log.entries.length < self.opts.queue.entries) {
      debug('[agent] queued (%d/%d)', self.message.har.log.entries.length, self.opts.queue.entries)

      return done()
    }

    debug('[agent] sending (%d)', self.message.har.log.entries.length)
    ...
    request.on('response', function (res) {
      ...
      res.on('end', function () {
        debug('[socket] %d %s: %s', res.statusCode, res.statusMessage, Buffer.concat(chunks))
    ...
    request.on('error', function (err) {
      debug('[agent] problem with request: %s', err.message)
  ...
  return function (req, res, next) {
    debug('[agent] triggered')

    var reqReceived = new Date()

    self.clientIPAddress = req.connection.remoteAddress ||
