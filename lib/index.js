 ...
 * Agent
 *
 * serviceToken - account service token
 * environment - environment
 * options:
 * - host - socket server hostname
 * - port - socket server port
 * - bodySizeMax - body size restriction
 * - queueBatch - number of batches to send per request
 * - queueEntries - number of entries per batch
 * - queueTimeout - request flush timeout (ms)
 */
module.exports = function Agent (serviceToken, environment, options) {
  ...
  return function (req, res, next) {
    ...
    if (self.opts.maxBodySize > 0) {
      req.on('data', function (chunk) {
        bytes.req += chunk.length

        if (bytes.req <= self.opts.maxBodySize) {
    ...
    res.write = function (chunk, encoding) {
      ...
      if (bytes.res <= self.opts.maxBodySize) {
      ...
      var entry = {
        ...
        request: {
          ...
          postData: {
            ...
            text: self.opts.maxBodySize > 0 ? bodies.req.base64 : '' // TODO
          }
        },
        ...
        response: {
          ...
          content: {
            ...
            text: self.opts.maxBodySize > 0 ? bodies.res.base64 : ''
          }
        },
